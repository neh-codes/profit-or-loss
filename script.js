...
function calculate(initial, curr, qty) {
  if (initial > curr) {
    ...
    output.innerText = `Oops!! You have incurred the loss of ₹${totalLoss} and loss percentage is ${lossPercentage.toFixed(
      2
    )}%`;
    ...
    output.innerText = `Yayy!! You have earned the profit of ₹${totalProfit} and profit percentage is ${profitPercentage.toFixed(
      2
    )}%`;
    document.body.style.backgroundColor = "#ceffe7";
  } else {
...
function validate() {
  ...
  else calculate(Number(initial), Number(curr), Number(qty));
}
